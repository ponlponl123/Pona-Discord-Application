...
import { constructTrack, Plugin, Structure, TrackUtils } from './utils';
...
export class Manager extends EventEmitter {
  ...
  public async loadPlayerStates(nodeId: string): Promise<Player | undefined> {
      ...
      if (
        ...
        if (state.dynamicRepeat)
          player.setDynamicRepeat(
            state.dynamicRepeat,
            state.dynamicLoopInterval._idleTimeout,
          );
        if (state.isAutoplay)
        ...
        console.log(
          ...
          consolePrefix.lavalink +
            `Loaded lavalink player state for ${state.options.guild}.`,
        );
      }
    }
    ...
    console.log(
      consoleType.info,
      consolePrefix.lavalink + 'Restored lavalink states from redis.',
    );
  }
  ...
  public async readPlayerState(guildId: string): Promise<Player | undefined> {
    ...
    if (state && typeof state === 'object' && state.guild) {
      ...
      console.log(
        consoleType.info,
        consolePrefix.lavalink +
          `Read lavalink player state for ${state.options.guild}.`,
      );
      return player;
    }
  ...
  public async savePlayerState(guildId: string): Promise<void> {
    ...
    console.log(
      consoleType.info,
      consolePrefix.lavalink +
        `Saved player state to "state:${guildId}" for ${guildId}`,
    );
  }
  ...
  public async delPlayerState(guildId: string): Promise<void> {
    ...
    console.log(
      consoleType.info,
      consolePrefix.lavalink +
        `Deleted player state from "state:${guildId}" for ${guildId}`,
    );
  }
  ...
  private async collectPlayerStateEvent(
    event: keyof ManagerEvents,
    player: Player,
  ): Promise<void> {
    ...
    if (!this.eventBatchInterval) {
      this.eventBatchInterval = setTimeout(
        () => this.processBatchEvents(),
        this.eventBatchDuration,
      ) as NodeJS.Timeout;
    }
  }
    ...
    try {
      ...
      if (res.loadType === 'playlist') {
        playlist = {
          ...
          tracks: playlistData!.tracks.map(
            async (track: TrackData) => await constructTrack(track, requester),
          ),
      ...
      const result: SearchResult = {
        ...
        playlist: playlist
          ? ({
              ...playlist,
              tracks: await Promise.all(playlist.tracks),
            } as PlaylistData)
          : undefined,
      };
      if (this.options.replaceYouTubeCredentials) {
        ...
        if (result.loadType === 'playlist')
          (result.playlist as PlaylistData).tracks = (
            result.playlist as PlaylistData
          ).tracks.map(processTrack);
  ...
  public create(options: PlayerOptions): Player {
    if (this.players.has(options.guild))
      return this.players.get(options.guild) as Player;
  ...
  public createNode(options: NodeOptions): Node {
    if (this.nodes.has(options.identifier || options.host))
      return this.nodes.get(options.identifier || options.host) as Node;
  ...
  public async updateVoiceState(
    ...
    if (
      't' in data &&
      !['VOICE_STATE_UPDATE', 'VOICE_SERVER_UPDATE'].includes(data.t as string)
    )
      return;
    ...
    console.log(
      ...
      consolePrefix.lavalink,
      player.guild,
      'Player destroyed by updateVoiceState event.',
    );
    return;
  }
}
