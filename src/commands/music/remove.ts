    ...
    if (playback && playback.queue.current) {
      ...
      for (let i = 0; i < total_pages; i++) {
        const selector = new StringSelectMenuBuilder()
          ...
          .setPlaceholder(
            `${lang.data.music.queue.remove.selector_placeholder}${total_pages > 1 && `(${lang.data.music.queue.remove.page_num} #${i + 1})`}`,
          )
          .addOptions(
            playback.queue
              .slice(i * 24, (i + 1) * 24)
              .filter((track) => track.uniqueId !== currentTrack.uniqueId)
              .map((track, index) => {
                return new StringSelectMenuOptionBuilder()
                  .setValue(String(index + i * 24))
                  .setLabel(
                    `${index + i * 24 + 1}. ${track.title}`.slice(0, 100),
                  );
              }),
          );
      ...
      collector.on(
        'collect',
        async (collected: StringSelectMenuInteraction<CacheType>) => {
          const selectedId = collected.values[0];
          const selected = playback.queue.filter(
            (track) => String(track.uniqueId) === selectedId,
          );
          ...
          if (selected.length > 0) {
            ...
            if (
              ...
              let minVotes =
                Math.floor(voiceChannelMembersSize_NotABot.size / 2) + 1;
              const response = await interaction.followUp({
                poll: {
                  question: {
                    text: lang.data.music.queue.remove.poll
                      .replace('[track_name]', selected[0].title)
                      .replace('[min_votes]', String(minVotes)),
                  },
                  answers: [
                    {
              ...
              const listeningAdd = async (
                answer: PollAnswer | PartialPollAnswer,
                userId: string,
              ) => {
                if (
                  !('poll' in answer) ||
                  answer.poll.message.id !== response.id
                )
                  return;
                if (
                ...
                if (voters.size >= minVotes) {
                  ...
                  answer.poll.answers.forEach(
                    async (ans: PollAnswer | PartialPollAnswer) => {
                      if (
                        'text' in ans &&
                        ans.text === lang.data.music.queue.remove.approved &&
              ...
              container.pona.client.once(Events.MessagePollVoteAdd, listeningAdd);
              container.pona.client.once(Events.MessagePollVoteRemove, listeningRm);
              return;
            } else {
            ...
            await interaction.editReply({
              content: lang.data.components.string_selector.error.invalid,
            });
          }
          collector.stop();
        },
      ...
      const remove = async (track: Track, voted: boolean) => {
        const index = playback.queue.findIndex(
          (findtrack) => findtrack.uniqueId === track.uniqueId,
        );
        ...
        const removedEmbed = new EmbedBuilder()
          .setAuthor({
            ...
            iconURL:
              'https://cdn.discordapp.com/emojis/1299947520952238130.webp?size=32&quality=lossless',
          })
