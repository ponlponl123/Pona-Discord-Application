...
export class Player {
  ...
  public search<T = User | ClientUser>(
  ...
  public async play(
    ...
    playOptions?: Interface.PlayOptions,
  ): Promise<void> {
    if (
    ...
    if (TrackUtils.isUnresolvedTrack(this.queue.current)) {
      try {
        this.queue.current = await TrackUtils.getClosestTrack(
          this.queue.current as Interface.UnresolvedTrack,
        );
      } catch (error) {
        this.manager.emit('trackError', this, this.queue.current, error);
        if (this.queue[0]) return this.play(this.queue[0]);
        console.error(
          `${prefix} No more tracks in the queue to play after an error.`,
          error,
        );
        return;
      }
  ...
  public async getRecommended<T = User | ClientUser>(
    ...
    if (hasSpotifyURL) {
      ...
      if (isSpotifyPluginEnabled && isSpotifySourceManagerEnabled) {
        ...
        const artistID = node.extractSpotifyArtistID(
          track.pluginInfo.artistUrl as string,
        );
        let identifier = '';
        if (trackID && artistID) {
        ...
        if (identifier) {
          ...
          if (recommendedResult.loadType === 'playlist') {
            ...
            if (recommendedTracks) {
              const tracks = recommendedTracks.map((track) =>
    ...
    const filteredTracks = tracks.filter(
      (track) => track.uri !== `https://www.youtube.com/watch?v=${videoID}`,
    );
  ...
  public previous(): this {
    const oldPlayer = { ...this };
    this.queue.unshift(
      this.queue.previous as Interface.Track | Interface.UnresolvedTrack,
    );
    this.stop();
    this.manager.emit('playerStateUpdate', oldPlayer, this, 'trackChange');
    return this;
  ...
  public seek(position: number): this | undefined {
    ...
    if (position < 0 || position > (this.queue.current.duration as number))
      position = Math.max(
        Math.min(position, this.queue.current.duration as number),
        0,
      );
