  ...
  try {
    ...
    if (!userVoiceChannel) {
      const embed = new EmbedBuilder()
        .setDescription(
          `<:X_:1298270493639446548> · **${lang.data.reasons.invalid_voice_channel}**!`,
        )
        .setFooter({
    ...
    const currentConnectionInGuild = await isPonaInVoiceChannel(
    ...
    if (currentConnectionInGuild) {
      ...
      if (await leaveVoiceChannelAsPlayer(currentConnectionInGuild.guild)) {
        const embed = new EmbedBuilder()
          .setDescription(
            `<:Check:1298270444150980619> · **${lang.data.music.play.leaved}**!`,
          )
          .setColor('#F9C5D5');
    ...
    const embed = new EmbedBuilder()
      .setDescription(
        `<:X_:1298270493639446548> · **${lang.data.music.errors.pona_not_in_voice_channel}**!`,
      )
      .setColor('#F2789F');
