...
export default new Elysia()
  .get('/subscribe', async ({ headers, query, set }) => {
    try {
  ...
  .get('/subscribe/:options', async ({ headers, params, query, set }) => {
    try {
      if (!database || !database.pool) {
        set.status = HttpStatusCode.ServiceUnavailable;
        return { error: 'Service Unavailable' };
  ...
  .post('/subscribe', async ({ headers, query, set }) => {
    try {
      if (!database || !database.pool) {
        set.status = HttpStatusCode.ServiceUnavailable;
        return { error: 'Service Unavailable' };
  ...
  .delete('/subscribe', async ({ headers, query, set }) => {
    try {
      if (!database || !database.pool) {
        set.status = HttpStatusCode.ServiceUnavailable;
        return { error: 'Service Unavailable' };
      ...
      if (redisClient?.redis)
        redisClient.redis.hset(`user:${user.id}:subscribe`, channelId, 0),
          redisClient.redis.expire(`user:${user.id}:subscribe`, 86400);
      database.query(`DELETE FROM subscribe_artist WHERE uid=? AND target=?`, [
        user.id,
        channelId,
      ]);
      set.status = HttpStatusCode.Ok;
