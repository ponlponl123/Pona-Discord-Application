        ...
        switch (state) {
          case 'track': {
            if (
              ...
              const repeatStateEmbed = new EmbedBuilder()
                .setTitle(
                  `<:Revertarrow:1299947479571107942> · ${lang.data.music.state.repeat.title}: ${lang.data.music.state.repeat.track}`,
                )
          ...
          case 'queue': {
            if (
              ...
              const repeatStateEmbed = new EmbedBuilder()
                .setTitle(
                  `<:MusicNote:1299943220301529118> · ${lang.data.music.state.repeat.title}: ${lang.data.music.state.repeat.queue}`,
                )
                .setColor(color('focus'));
              return interaction.reply({
                embeds: [repeatStateEmbed],
          ...
          default: {
            if (
              ...
              if (
                (await loopQueueSubsystem(interaction, false, false)) ===
                  true &&
                interaction.isRepliable()
              ) {
                const repeatStateEmbed = new EmbedBuilder()
                  .setTitle(
                    `<:Revertarrowwithslash:1299947493756243989> · ${lang.data.music.state.repeat.title}: ${lang.data.music.state.repeat.off}`,
                  )
        ...
        return interaction.reply({
          embeds: [
            await errorEmbedBuilder(
              member.guild.id,
              lang.data.errors.invalid_subcommand,
            ),
          ],
        });
    }
