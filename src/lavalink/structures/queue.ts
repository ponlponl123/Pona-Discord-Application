...
export class Queue extends Array<Track | UnresolvedTrack> {
  ...
  public add(
    ...
    const addTrack = (t: Track | UnresolvedTrack) => {
      if (
        !this.some((existingTrack) => existingTrack.uniqueId === t.uniqueId)
      ) {
        this.push(t);
      }
    };
    ...
    if (!this.current) {
      ...
      if (typeof offset !== 'undefined' && typeof offset === 'number') {
        ...
        if (Array.isArray(track))
          track.forEach((t) => this.splice(offset, 0, t));
    ...
    this.manager.emit(
      'playerStateUpdate',
      oldPlayer,
      this.manager.players.get(this.guild),
      'queueChange',
    );
  }
  ...
  public move(from: number, to: number): void {
    ...
    if (isNaN(Number(from)) || isNaN(Number(to)))
      throw new RangeError(`Missing "from" or "to" parameter.`);
  ...
  ): (Track | UnresolvedTrack)[] | undefined {
    const oldPlayer = { ...this.manager.players.get(this.guild) };
